...
train_dataset = TensorDataset(train_spikes, train_behavior)
val_dataset = TensorDataset(val_spikes, val_behavior)
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)  # Default batch size for initialization
val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)  # Default batch size for initialization

#%% -----------------------------------
# Run a single run
# -------------------------------------
wandb.init(project=WANDB_PROJECT, entity=WANDB_ENTITY, name=f'RNN_0')


# Set up the RNN model, optimizer, and loss function
model = RNNMODEL(input_size=input_size, rnn_layers=RNN_LAYERS[0], rnn_hidden_size=RNN_HIDDEN_SIZE[0], rnn_dropout=RNN_DROPOUT[0]).to(device)
optimizer = optim.Adam(model.parameters(), lr=RNN_LR[0])
mse_loss = nn.MSELoss()

# Training loop
for epoch in range(RNN_EPOCHS[0]):
    model.train()
    total_loss = 0
...
#     "method": "grid",
#     "parameters": {
#         "rnn_layers": {"values": RNN_LAYERS},
#         "rnn_hidden_size": {"values": RNN_HIDDEN_SIZE},
#         "rnn_dropout": {"values": RNN_DROPOUT},
#         "rnn_lr": {"values": RNN_LR},
#         "rnn_epochs": {"values": RNN_EPOCHS},
#         "rnn_batch_size": {"values": RNN_BATCH_SIZE}
#     }
# }

# # Initialize the sweep
# sweep_id = wandb.sweep(sweep=sweep_config, project="rnn-sweep-project")

# def train():
#     # Iterate over sweep configurations
#     for config in wandb.controller():
...
#         optimizer = optim.Adam(model.parameters(), lr=config['rnn_lr'])
...
#         # Log the sweep configuration and final metrics to Weights and Biases
#         wandb.log(config)
